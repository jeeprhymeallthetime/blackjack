...
def end_of_hand_split(busted, player_hand, dealer_hand, shuffled_deck, bet, scenario_num, busted_one, busted_two, busted_three, busted_four, hand_one, hand_two, hand_three, hand_four):
    result = 0
    if busted is False:
        dealer_hand, shuffled_deck = act_funcs.dealers_turn(dealer_hand, shuffled_deck)
        if scenario_num == 1:
            ...
            if act_funcs.score_hand(dealer_hand) > 21:
                print("Dealer busts! Everybody wins")
                result = 0
            elif act_funcs.score_hand(hand_one) > act_funcs.score_hand(dealer_hand):
                print("Player's Hand #1 wins!")
                result = result + 1
            elif act_funcs.score_hand(dealer_hand) == act_funcs.score_hand(hand_one):
                print("Hand #1 Push")
                result = result + 1
            else:
                print("Player loses.")
            ...
            for i in range(0, len(hand_one)):
                hand_print += str(hand_one[i].value) + ' of ' + hand_one[i].suit + ', '
            print("Player Hand:", hand_print[:-2], '- Final Score: ', act_funcs.score_hand(hand_one))
            print("Hand #2")
            if act_funcs.score_hand(dealer_hand) > 21:
                print("Dealer busts! Everybody wins")
                result = 0
            elif act_funcs.score_hand(hand_two) > act_funcs.score_hand(dealer_hand):
                print("Player's Hand #2 wins!")
                result = result + 1
            elif act_funcs.score_hand(dealer_hand) == act_funcs.score_hand(hand_two):
                print("Hand #2 Push")
                result = result + 1
            else:
                print("Player loses.")
            ...
            print("Dealer Hand:", hand_print[:-2], '- Final Score: ', act_funcs.score_hand(dealer_hand))
            hand_print = ''
            for i in range(0, len(hand_two)):
                hand_print += str(hand_two[i].value) + ' of ' + hand_two[i].suit + ', '
            print("Player Hand:", hand_print[:-2], '- Final Score: ', act_funcs.score_hand(hand_two))
    else:
        result = 0
    return result
...
def play_game(shuffled_deck, wallet, p):
    ...
    while game_on is True:
        ...
        shuffled_deck, busted, split, scenario_num, busted_one, busted_two, busted_three, busted_four, hand_one, hand_two, hand_three, hand_four = start_of_hand(player_hand, dealer_hand, shuffled_deck, bet, p)

        if scenario_num == 0:
            result = end_of_hand(busted, player_hand, dealer_hand, shuffled_deck, bet)
        else:
            result = end_of_hand_split(busted, player_hand, dealer_hand, shuffled_deck, bet, scenario_num, busted_one, busted_two, busted_three, busted_four, hand_one, hand_two, hand_three, hand_four)
        wins = wins + result
        total = total + 1
        print(total)
